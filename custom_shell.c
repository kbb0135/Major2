...
void initialize_batch_mode()
	{
		string_variables = malloc(sizeof(char*)*100);
		int_variables = malloc(sizeof(char*)*100);

		//char **array = malloc(totalstrings * sizeof(char *));
		string_values = malloc(sizeof(char*)*100);
		string_count=0;
		int_count=0;
	}
...
void handle_operator(char* line)
	{
		...
		if (strchr(line, '+')!=NULL)						//if addition
			{
				//to keep track of the negative values
				bool mintest= false;
				char* tchd;
				tchd = malloc(sizeof(char)*512);
				...
				if (chd[0]=='-') { mintest = true; }			//if its a negative value
				...
				if (mintest==true) { total = total *-1; }		//making it negative
				chd = strtok(NULL,"\n");				//getting the second aka last variable
				if (chd==NULL) { return;}
				mintest= false;
				trimLeading(chd);
				if (chd[0]=='$') { remove_first(chd); }
				if (chd[0]=='-') { mintest = true; }
				if (!isnumber(chd) && !is_int_variable(chd))	//valid or not
						{
							return;
						}
				int temp = getvalue_of_int(chd);
				if (mintest==true) { temp = temp*-1; }			
				total = total + temp;			//gettting the variable value	
				values[int_count]=total;
				int_count++;						//increasing the count of integer variable
			}
		else if (strchr(line, '*')!=NULL) 										//for multiplication; everything same as addition
			{
				bool mintest= false;
				char* tchd;
				tchd = malloc(sizeof(char)*512);
				...
				if (mintest==true) { total = total *-1; }			//if the value is negative
				chd = strtok(NULL,"\n");
				if (chd==NULL) { return;}
				mintest=false;
				trimLeading(chd);
		...
		else if (strchr(line, '-')!=NULL)					//for subtraction everything same as addition
			{
				...
				tchd = strtok(NULL,"-");
				trimLeading(tchd);
				//if we got a subtraction in s ubtraction like ( a = -20 -20) or if we hava an assignment like a=-20
				if (line[strlen(new_var)+1]=='-') { mintest = true;}
				if (tchd==NULL || strlen(tchd) == 0 ) 
					{
 						tchd = strtok(NULL,"-");
						trimLeading(tchd);
						mintest = true;
					}
				if (tchd[0]=='$') { remove_first(tchd); }
					if (!isnumber(tchd) && !is_int_variable(tchd))
						{
							return;
						}

				int total = getvalue_of_int(tchd);

				//multiply by -1 to get the number
				if (mintest ==true) { total = total*-1; }
				mintest = false;
				chd = strtok(NULL,"\n");
				//printf("%d\n",strlen(chd);
				//if nothing else to assign
				if (chd==NULL) 
					{
						values[int_count]=total;
						int_count++;
...
void print_variable(char* c)
	{	
		...
		//if nowhere found
		printf("%s variable not declared/initialized\n",c);
	}
